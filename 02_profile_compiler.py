...
def _():
    ...
    return ExperimentStore, ProfileCompiler, ProfileDraft, json, json_view, mo, os, store
...
@app.cell
def _(ProfileCompiler, api_key, compile_profile, existing_profile, json, json_view, max_output, mo, model, os, prompt_version, reasoning, store, transcript):
    ...
    try:
        with mo.status.spinner(title="Compiling profile draft…"):
        ...
        compilation_view = mo.vstack([
            ...
            accept,
            mo.accordion({
                "Exact API request": json_view(compiled["trace"]["request"]),
                "Raw model response": json_view(compiled["raw_response"]),
                "Standard trace": json_view(compiled["trace"]),
            }),
        ...
        accept = None
        draft_editor = None
        compilation_view = mo.callout(f"Profile compilation failed: `{type(exc).__name__}: {exc}`", kind="danger")
    compilation_view
    return accept, draft_editor


@app.cell
def _(ProfileDraft, accept, draft_editor, json, json_view, mo):
    mo.stop(accept is None or draft_editor is None or not accept.value)
    try:
        accepted = ProfileDraft.from_dict(json.loads(draft_editor.value)).to_dict()
        acceptance_view = mo.vstack([mo.callout("Edited profile is valid and accepted for the next explicit persistence/embedding step.", kind="success"), json_view(accepted)])
    except Exception as exc:
