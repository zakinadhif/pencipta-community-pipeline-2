...
class ExperimentStore:
    ...
    def _connection(self):
        """Open DuckDB only for one operation so other notebook processes can use it."""
        ...
        while True:
            try:
                ...
                if "used by another process" not in str(exc) or time.monotonic() >= deadline:
                    raise
                time.sleep(0.1)
        try:
            yield con
    ...
    def add_call(self, run_id: str, trace: dict[str, Any]) -> None:
        ...
        with self._connection() as con:
            con.execute("insert into llm_calls values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [
    ...
    def add_retrieval(self, run_id: str, rows: list[dict[str, Any]]) -> None:
        with self._connection() as con:
            for row in rows:
                con.execute("insert into retrieval_results values (?, ?, ?, ?, ?, ?, ?, ?, ?)", [
    ...
    def add_evaluation(self, run_id: str, candidate_id: str, rating: str, notes: str) -> None:
        with self._connection() as con:
            con.execute("insert into human_evaluations values (?, ?, ?, ?, ?, ?)",
        ...
        with self._connection() as con:
            for bucket in buckets:
                for result in bucket.get("results", []):
                    con.execute("insert into authoritative_cost_buckets values (?, ?, ?, ?, ?, ?, ?, ?)", [
