...
def per_run_tokens(store: ExperimentStore) -> list[dict[str, Any]]:
    calls = store.dataframe("""
        select llm_calls.run_id, runs.requester_id, runs.query,
        ...
        order by llm_calls.run_id
    """)
    rows = []
    for run_id, group in calls.groupby("run_id", sort=False):
        base = {"run_id": run_id, "requester_id": group["requester_id"].iloc[0], "query": group["query"].iloc[0]}
